...
class ProductProvider extends Component {
  ...
  updateFilters = async (filter, option) => {
    ...
    if (filterFound) {
      let filteredFilters = this.state.cart.filter(
        (filter) => filter.value === filter
      );
      this.setState(() => {
        return {
          filters: [...filteredFilters, { filter: filter, option: option }],
        };
      });
    } else {
      this.setState(() => {
        return {
          filters: [...this.state.filters, { filter: filter, option: option }],
        };
      });
    }
  };
...
registerUser = async (newUser) => {
    if (! (await this.userExists(newUser.email))) {
      console.log(newUser);
      await post("http://localhost:3005/users/", newUser);
      return true;
    } else {
      return false;
    }
