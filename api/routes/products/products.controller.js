...
async function updateProduct(req, res) {
  try {
    ...
    let bodyValidated = {};
    if (body.arrival) {
      bodyValidated.arrival = body.arrival;
    }
    if (body.breed) {
      bodyValidated.breed = body.breed;
    }
    if (body.delivery) {
      bodyValidated.delivery = body.delivery;
    }
    if (body.description) {
      bodyValidated.description = body.description;
    }
    if (body.image) {
      bodyValidated.image = body.image;
    }
    if (body.offer) {
      bodyValidated.offer = body.offer;
    }
    if (body.package) {
      bodyValidated.package = body.package;
    }
    if (body.price) {
      bodyValidated.price = body.price;
    }
    if (body.size) {
      bodyValidated.size = body.size;
    }
    if (body.sterilized) {
      bodyValidated.sterilized = body.sterilized;
    }
    if (body.stock) {
      bodyValidated.stock = body.stock;
    }
    if (body.title) {
      bodyValidated.title = body.title;
    }
    if (body.type) {
      bodyValidated.type = body.type;
    }
    if (body.vendor) {
      bodyValidated.vendor = body.vendor;
    }
...
async function getProductsByCategory(req, res) {
  try {
    ...
    let typeCategory = allCategories.filter(
      (category) => category.value.toLowerCase() == "type"
    )[0];

    if (
      !typeCategory.options.some((option) => {
        let trimmedOption = option.toLowerCase().replaceAll(" ", "");
        console.log(trimmedOption == optionName.toLowerCase())
        return trimmedOption == optionName.toLowerCase();
      })
    ) {
    ...
    let filteredProducts = allProductsResponse.filter((product) => {
      let trimmedType = product.type.toLowerCase().replaceAll(" ", "");
      return trimmedType == optionName.toLowerCase().trim();
    });
